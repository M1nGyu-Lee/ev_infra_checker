...
| 3순위 | 기초 | EV·충전소 총량 | 전국 EV·환경부 급·완속 현황 구축 규모 |
...
| 급·완속 설치 판단 | 급속 비중·보급 강도·활성기 | 환경부 급·완속 현황 processed + master + annual |
| EV·충전소 총량 | 전국 KPI | master, `ev_sido_monthly`, me_charger_status annual |
...
| `data/raw/me_charger_status/*` | me_charger_status_* | 급속 비중·8권역 구축 |
| GeoJSON light | (정적) | 지도 |
...
- 브리핑: 연도·지도 지표 선택. 환경부 급·완속 현황는 펼침(참고). 결론 KPI는 국토부·환경부.
- 탐색 화면: 사이드바 기준연도·지역
